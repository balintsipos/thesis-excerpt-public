...
def main():
    ...
    for i in range(1):
        ...
        X_test_backdoors = pd.DataFrame(x_scaled)
    ...
    # ---------------------------------------------------similar
        ...
        print("all statistics models created")
    # ---------------------------------------------------similar

        extracted_predict_proba = extracted_logreg.predict_proba(X_test_backdoors.values)
        

        for x in extracted_predict_proba:
            with open("statistics_extracted_results2.txt", 'a') as f:
                result = str(round(float(x[1]),3))
                f.write(result+'\n')

        similar_predict_proba = similar_logreg.predict_proba(X_test_backdoors.values)
        

        for x in similar_predict_proba:
            with open("statistics_similar_results2.txt", 'a') as f:
                result = str(round(float(x[1]),3))
                f.write(result+'\n')
...
#--------------------------------
    ...
    separate_extracted_predict_proba = extracted_logreg.predict_proba(X_test_backdoors.values)
        

    for x in separate_extracted_predict_proba:
        with open("separate_extracted_results2.txt", 'a') as f:
            result = str(round(float(x[1]),3))
            f.write(result+'\n')
